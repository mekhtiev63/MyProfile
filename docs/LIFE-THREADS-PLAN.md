...
### Фаза 2 — Нить в контенте ✅ в работе

**Цель:** Highlights и метрики — продолжение той же нити, не отдельный блок.

- [x] `ThreadSpine` — scroll-linked вертикальная нить через секции (desktop)
- [x] Метрики как узлы нити (count-up + node dot на карточках)
- [x] `About` — anchor фраза из профиля нити + узел схождения
- [x] Parallax hero canvas при скролле
- [x] Удалены legacy `HeroBackdrop`, `HeroCanvas`, `AbstractScene`

**Критерий:** скролл ощущается как движение по одной нити.
